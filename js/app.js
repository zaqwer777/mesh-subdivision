...
class MeshObject {
    constructor(boundingBox, scaleFactor, center, numElements, arrays, buffers) {
...
window.onSubdivide = () => {
    ...
    function addScaledVec(posVec, scale, vec) {
        var scaledVec = [];
    ...
    function bulge() {
        ...
        for (var i = 0; i < nc; i++) {
            if (i < object.getOpposite(i)) { // no tweak for mid-vertices of border edges
                ...
                var midB = midPt(vecG[object.getVertex(object.getLeft(object.getOpposite(i)))], vecG[object.getVertex(object.getRight(object.getOpposite(i)))]);
